...
- **For the remaining checklist gaps specifically, don't build a real backend for every one of them.** Pick one or two genuinely worth making functional per area (small, reuses existing infra, teaches something distinct — same test as above) and **mock the rest at the UI layer only**: a form that shows a success toast/state on submit without ever calling a real endpoint, a listing/dashboard that renders fixed sample data, an "export" button that simulates completion — no `apps/api` route, no persistence, nothing to verify server-side. This is not a lesser version of the feature to "finish later" — it's the deliberately final form for that screen in this project, exactly like the already-hardcoded scheduling constants or the never-built settings-editing UI.
- **Every mocked screen/flow must say so, visibly, in one place a reviewer will actually find**: a short "O que é real vs. simulado" section in `README.md` (see "Keeping README.md current" below) listing, per screen or flow, which endpoints are genuinely implemented and which are UI-only simulations — plus a one-line comment at the mock's call site in code (e.g. `// mocked: no backend, see "O que é real vs. simulado" in README.md`). The point isn't to hide the simplification, it's to make the *demonstrated understanding* visible: "I know what this flow needs, here's proof for endpoints X/Y/Z, and here's an honest UI-only stand-in for the rest so the app doesn't feel broken."
...
This is a Yarn workspaces monorepo for **AutoAgenda**. `apps/web` (Vite + React 19 + TypeScript + TailwindCSS) and `apps/api` (Node.js/Express) both cover every profile the academic spec describes (Admin, Instrutor, Aluno) — see [CHECKLIST.md](CHECKLIST.md) for the current per-profile state and README.md's "O que é real vs. simulado" for which pieces are genuinely real vs. deliberately mocked at the UI layer. `packages/contracts` (shared schemas/types) is still an empty placeholder. The sections below define this project's actual architecture/conventions — read them as descriptive of what exists, not aspirational.
...
- `src/database/migrations/*.sql` — plain numbered SQL migrations (`0001_create_user.sql`, `0002_create_session.sql`, `0003_create_license_category.sql`, `0004_create_student.sql`, `0005_create_vehicle.sql`, `0006_create_instructor.sql`, `0007_create_appointment.sql`, `0008_create_password_reset_token.sql`, `0009_create_instructor_availability.sql`, `0010_create_instructor_block.sql` so far), applied in filename order by `scripts/migrate.ts`, which tracks what's already applied in a `_migrations` bookkeeping table it creates itself. Not a migration framework — a small custom runner, consistent with the "plain SQL, no ORM" stance below.
...
- `src/http/routes/studentRoutes.ts` + `src/http/controllers/studentController.ts` — `GET/POST /students`, `GET/PATCH /students/:id`, `POST /students/:id/deactivate` (all behind `requireAuth` + `requireRole('ADMIN')`). Backed by `src/modules/students/studentService.ts` (pagination/filtering envelope, document-uniqueness conflict handling, category validation) and `src/repositories/{studentRepository,licenseCategoryRepository}.ts`. Students are inactivated, never deleted (`docs/05` §5) — there's no `DELETE /students/:id`. `POST /students/:id/create-account` (Admin-only) lets an Admin grant an *existing* student login access — a `user` insert (`role = 'STUDENT'`) plus `studentRepository.linkUserId`, wrapped in a transaction mirroring `instructorService.register`'s shape; rejects with `409 STUDENT_ALREADY_HAS_ACCOUNT` if `student.user_id` is already set. This reverses the project's earlier "students never get login accounts" decision (`student-self-service` change) — students are still Admin-enrolled first (as always), gaining portal access is now a separate, optional, later step, not self-registration. `GET/PATCH /students/me` (behind `requireAuth` + `requireRole('STUDENT')`) mirror `/instructors/me` exactly — self-service limited to `phone`, resolved via `studentRepository.findByUserId(req.user.id)`, registered before `/students/:id` for the same route-ordering reason. `GET /license-categories` (a static A/B/AB/C/D/E lookup, no sensitive content) is behind `requireRole('ADMIN', 'STUDENT')`, not Admin-only, so Student > Perfil can resolve its own `category_id` to a readable code.
...
- `src/http/routes/instructorRoutes.ts` + `src/http/controllers/instructorController.ts` — `GET/POST /instructors`, `GET/PATCH /instructors/:id` (all behind `requireAuth` + `requireRole('ADMIN')`, same no-dedicated-deactivate reasoning as vehicles). Backed by `src/modules/instructors/instructorService.ts` and `src/repositories/instructorRepository.ts` (its `findMany`/`findById` `JOIN user` for the linked login `email`, read-only). Registering an instructor is the project's first cross-table write: `instructorService.register` receives the raw `db` handle alongside `userRepository`/`instructorRepository` and wraps a `user` insert (`role = 'INSTRUCTOR'`, Admin-set initial password) and the `instructor` insert in an explicit `db.exec('BEGIN'/'COMMIT'/'ROLLBACK')` transaction — a deliberate deviation from the pure repository-only service pattern `studentService`/`vehicleService` use, justified by needing atomicity a single repository call can't provide. `PATCH /instructors/:id` only ever touches profile fields (`full_name`/`document`/`credential_number`/`phone`/`status`) — `email`/password are silently ignored if present in the body, since no endpoint anywhere in this project changes login credentials yet. `GET /instructors/me` and `PATCH /instructors/me` (behind `requireAuth` + `requireRole('INSTRUCTOR')`, not `'ADMIN'` — Admin already has `/instructors/:id`) let an instructor view/edit their own record, resolved via `instructorRepository.findByUserId(req.user.id)` rather than a route param; `updateOwnProfile` only ever applies `phone`, silently ignoring any other field in the body, deliberately narrower than the Admin-facing update. **Both `me` routes are registered before `/instructors/:id` in `instructorRoutes.ts`** — Express matches route registration order, and `:id` would otherwise capture the literal string `"me"` as an id. `GET/POST /instructors/:id/availability` and `GET/POST /instructors/:id/blocks` (behind `requireAuth` + `requireRole('ADMIN', 'INSTRUCTOR')`) let the Admin (any instructor) or the owning instructor (their own `:id`) declare weekly recurring availability windows (`instructor_availability`: weekday 0-6, start/end time) and one-off unavailable intervals (`instructor_block`: start/end timestamp, reason) — backed by a separate `src/modules/instructors/instructorAvailabilityService.ts` (not folded into `instructorService.ts`) whose `assertCanManage` ownership check mirrors `getOwnProfile`'s "resolve via `findByUserId`" shape, just comparing against the route's `:id` instead of always resolving to self. No `PATCH`/`DELETE` for either sub-resource — `docs/04` §5 only lists `GET/POST`, and a wrong entry is worked around by adding a correcting one rather than editing/removing (`instructor-availability` change).
...
- `src/http/routes/appointmentRoutes.ts` + `src/http/controllers/appointmentController.ts` — `GET /availability/slots` and `POST /appointments` behind `requireAuth` + `requireRole('ADMIN', 'STUDENT')`; `GET /appointments` behind `requireAuth` + `requireRole('ADMIN', 'INSTRUCTOR', 'STUDENT')` — implementing `docs/04` §5's "Por perfil": Admin sees/searches/books for anyone, an Instructor sees only their own appointments, a Student sees only their own and can only search/book for themselves, all scoped server-side inside `appointmentService` (which resolves the caller's own `instructor.id`/`student.id` via `instructorRepository.findByUserId`/`studentRepository.findByUserId(req.user.id)`, returning an empty list rather than erroring if no linked record is found for `list`). For a `STUDENT` caller, `searchSlots`/`book` take a `requester: Requester` parameter and **override any client-supplied `studentId`/`categoryId`** with the resolved student's own id/category — a student can never search or book on behalf of another student or outside their own registered category (`student-self-service` change); the frontend self-service form (`Student/ScheduleClass`) accordingly never collects those two fields. Every `GET /appointments` response includes `student_full_name`/`instructor_full_name`/`vehicle_plate` resolved via a `JOIN` in `appointmentRepository`, since neither an Instructor nor a Student has access to `/students`/`/instructors`/`/vehicles` to resolve those names any other way. Backed by `src/modules/appointments/appointmentService.ts` — the scheduling algorithm, deliberately scoped down from `docs/04` §8's full 10-step version per the "Academic scope & delivery philosophy" above: `BUSINESS_HOURS_START/END_HOUR`, `DEFAULT_DURATION_MINUTES`, `MIN_ADVANCE_MINUTES`, `SLOT_STEP_MINUTES`, `MAX_SLOTS_RETURNED`, and `MAX_SEARCH_RANGE_DAYS` are hardcoded constants in that file standing in for a `system_setting` table this project doesn't have; per-instructor category authorization and per-vehicle unavailability windows are still not modeled (every `ACTIVE` instructor is treated as authorized for every category; `vehicle.status = 'MAINTENANCE'` is the only unavailability mechanism relied on). Instructor weekly-availability windows and blocks **are** modeled and enforced (`instructor-availability` change): `searchSlots`'s per-candidate-instructor filter and `book`'s validation both call a local `isInstructorAvailable(instructorId, start, end)` closure — true only if a matching `instructor_availability` row covers that weekday/time-of-day (via `instructorAvailabilityRepository.findActiveByInstructorAndWeekday`, same UTC-only weekday/hour convention as `isWithinBusinessHours`) and no `instructor_block` row overlaps (`instructorBlockRepository.isBlocked`, same overlap-check param-order shape/gotcha as `appointmentRepository`'s `isFree`). **An instructor with zero declared availability windows is never offered/bookable** — this replaced the earlier "every active instructor is always available" simplification, which is why the demo seed (`seedDemoInstructors`) now also inserts Mon-Fri 08:00-18:00 windows for the two seeded instructors. `searchSlots` generates candidate times and resolves the first available instructor+vehicle pair per candidate; `book` re-validates everything and wraps the overlap check + insert in a synchronous (no `await`) `db.exec('BEGIN'/'COMMIT'/'ROLLBACK')` transaction — Node's single-threaded event loop, not a DB-level lock, is what makes that safe against concurrent requests here. No lifecycle actions (reschedule/cancel/confirm/attendance/complete/not-performed) exist yet — every appointment stays in `AGENDADA`. **Overlap-check gotcha already hit once**: `appointmentRepository.ts`'s per-resource overlap query binds `(resourceId, startAt, endAt)` in that exact order — swapping `startAt`/`endAt` silently inverts the whole conflict check (an identical double-booking would succeed instead of being rejected) with no type error to catch it; caught only by manually testing "book the same slot twice" during this change's own verification.
...
- No appointment lifecycle actions (reschedule/cancel/confirm/attendance/complete/not-performed) for any role — every appointment stays in `AGENDADA` regardless of who booked it, a standing Non-Goal since `appointment-scheduling`.
- Settings, audit-event querying, and reports (`docs/04` §5's `/settings`, `/audit-events`, `/reports/...`) are deliberately **mocked at the UI layer only** (`admin-mocked-screens` change), not real endpoints — see "O que é real vs. simulado" in `README.md`. Don't assume a real `apps/api` route exists for any of these.
